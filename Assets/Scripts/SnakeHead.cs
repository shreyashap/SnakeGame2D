...
public class SnakeHead : BodyPart
{
    ...
    [SerializeField] private AudioSource[] gulpSounds = new AudioSource[3];
    [SerializeField] private AudioSource dieSound;

    [SerializeField] private ParticleSystem eggParticle;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if(egg)
        {
            ...
            EatEgg(egg);
            eggParticle.transform.position = egg.transform.position;
            eggParticle.Play();
        }
        else
        {
